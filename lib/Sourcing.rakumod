no precompilation;
my %types;
class Sourcing does Associative {
	has %.funcs;
	has %.commands;
	has &.handler is rw;
	has &.emitter is rw;
	...
	role Event[Str $type where { "command" | "event" }] {
		method TWEAK(|) {
			.(self) with Sourcing.instance($type).emitter
		}
	}
...
multi EXPORT(--> Map()) {
	...
	'&EXPORT' => sub (@nss = ("",) --> Map()) {
		...
		|@nss.map: -> $ns {
			|Sourcing.instance<command>.get-funcs([ $ns.join("::").split: "::" ]).map: -> (:$key, :$value) {
				"&$key" => sub (*%named) {
					%cmds{$key} //= do {
						my $cmd-name = "{ $key.tc.subst: /\W(\w)/, { $0.uc }, :g }Command";
						my $cmd = Metamodel::ClassHOW.new_type: :name($cmd-name);
						$cmd.^add_role: Sourcing::Event["command"];
